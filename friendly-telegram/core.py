...
class TestManager:
    ...
    async def init(self):
        ...
        if stage > 4:
            stage = 0
            await self._db.set(__name__, "stage", stage)
        client_id = [(await c.get_me(True)).user_id for c in self._clients if c is not self._client][0]
        self._db.set(security.__name__, "bounding_mask", -1)
        if stage == 0:
            self._db.set(security.__name__, "bounding_mask", -1)
            self._db.set(security.__name__, "owner", [client_id])
            return ["usermodule.py"]
        self._db.set(security.__name__, "owner", [-1])
        if stage == 1:
            self._db.set(security.__name__, "sudo", [client_id])
            return ["usermodule.py"]
        if stage == 2:
            self._db.set(security.__name__, "sudo", [])
            self._db.set(security.__name__, "support", [client_id])
            return ["usermodule.py"]
        if stage == 3:
        ...
        if stage == 4:
            ...
            return ["usermodule.py"] + list(str(defmods))  # looks like that `defmods` is bytes, bot str expected
