...
class LoggerMod(loader.Module):
    ...
    async def append_entity(self, id_, entities, message):
        fail = True
        try:
            entity = await self._client.get_input_entity(id_)
        except ValueError:
            pass
        else:
        ...
        if fail:
            if not isinstance(id_, int):
                id_ = utils.get_entity_id(id_)
            entities.append(MessageEntityCode(len(message), len(str(id_))))
            message += str(id_)
        return message

    async def _log(self, type_, group, affected_uids, data):
